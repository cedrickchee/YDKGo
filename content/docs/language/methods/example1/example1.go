...
func main() {
	...
	// We are ranging over this slice of values, making a copy of each value
	// and call notify to make another copy.
	for _, u := range users {
		u.notify()
	}
	...
	// Exception example: Using pointer semantics for a collection of strings.
	keys := make([]string, 10)
	for i := range keys {
...
// Sending User Email To ed<ed@email.com>
// Sending User Email To erick<erick@email.com>
